...
type SizesPageProps = PageProps<{
    sizes: Pagination<SizeRow>;
}>;

export default function AdminSizesIndex() {
    const { sizes } = usePage<SizesPageProps>().props;
    ...
    const submit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (editingSize) {
            form.put(route('admin.sizes.update', editingSize.id), {
                preserveScroll: true,
                onSuccess: () => resetForm(),
            });
        } else {
    ...
    const toggleSize = (size: SizeRow) => {
        router.put(route('admin.sizes.update', size.id), {
            code: size.code,
            name: size.name,
            description: size.description,
            is_active: !size.is_active,
    ...
    const deleteSize = (size: SizeRow) => {
        setDeleteConfirm(size);
    };

    const handleDelete = () => {
        if (deleteConfirm) {
            router.delete(route('admin.sizes.destroy', deleteConfirm.id), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const handleBulkDelete = () => {
        router.delete(route('admin.sizes.bulk-destroy'), {
    ...
    const handlePerPageChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        router.get(route('admin.sizes.index'), { per_page: newPerPage }, { preserveState: true, preserveScroll: true });
    };
