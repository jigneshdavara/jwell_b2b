...
type MetalPuritiesPageProps = PageProps<{
    purities: Pagination<MetalPurityRow>;
    metals: MetalOption[];
}>;

export default function AdminMetalPuritiesIndex() {
    const { purities, metals } = usePage<MetalPuritiesPageProps>().props;
    ...
    const submit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (editingPurity) {
            form.put(route('admin.metal-purities.update', editingPurity.id), {
                preserveScroll: true,
                onSuccess: () => resetForm(),
            });
        } else {
            form.post(route('admin.metal-purities.store'), {
                preserveScroll: true,
                onSuccess: () => resetForm(),
            });
        }
    ...
    const togglePurity = (purity: MetalPurityRow) => {
        router.put(route('admin.metal-purities.update', purity.id), {
            metal_id: String(purity.metal_id),
            code: purity.code,
            name: purity.name,
            description: purity.description,
    ...
    const deletePurity = (purity: MetalPurityRow) => {
        setDeleteConfirm(purity);
    };

    const handleDelete = () => {
        if (deleteConfirm) {
            router.delete(route('admin.metal-purities.destroy', deleteConfirm.id), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const handleBulkDelete = () => {
        router.delete(route('admin.metal-purities.bulk-destroy'), {
    ...
    const handlePerPageChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        router.get(route('admin.metal-purities.index'), { per_page: newPerPage }, { preserveState: true, preserveScroll: true });
    };
            ...
            <div className="space-y-8">
                <div className="flex items-center justify-between rounded-3xl bg-white p-6 shadow-xl shadow-slate-900/10 ring-1 ring-slate-200/80">
                    <div>
                        ...
                        <p className="mt-2 text-sm text-slate-500">Manage metal purity grades for catalogue specifications.</p>
                    </div>
                    <button
                        type="button"
                    ...
                    <div className="flex-shrink-0 border-b border-slate-200 px-6 py-4">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                >
                                    {editingPurity ? 'Update metal purity' : 'Create metal purity'}
                                </button>
                                <button
                                    type="button"
                                    onClick={resetForm}
