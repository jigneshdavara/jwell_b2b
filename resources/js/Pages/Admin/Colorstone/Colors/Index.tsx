...
type ColorstoneColorsPageProps = PageProps<{
    colors: Pagination<ColorstoneColorRow>;
}>;

export default function AdminColorstoneColorsIndex() {
    const { colors } = usePage<ColorstoneColorsPageProps>().props;
    ...
    const openEditModal = (color: ColorstoneColorRow) => {
    ...
    const submit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (editingColor) {
            form.put(route('admin.colorstone.colors.update', editingColor.id), {
                preserveScroll: true,
                onSuccess: () => resetForm(),
            });
        } else {
            form.post(route('admin.colorstone.colors.store'), {
                preserveScroll: true,
                onSuccess: () => resetForm(),
            });
        }
    ...
    const toggleColor = (color: ColorstoneColorRow) => {
        router.put(route('admin.colorstone.colors.update', color.id), {
            code: color.code,
            name: color.name,
                        description: color.description,
            is_active: !color.is_active,
    ...
    const deleteColor = (color: ColorstoneColorRow) => {
        setDeleteConfirm(color);
    };

    const handleDelete = () => {
        if (deleteConfirm) {
            router.delete(route('admin.colorstone.colors.destroy', deleteConfirm.id), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const handleBulkDelete = () => {
        router.delete(route('admin.colorstone.colors.bulk-destroy'), {
    ...
    const handlePerPageChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        router.get(route('admin.colorstone.colors.index'), { per_page: newPerPage }, { preserveState: true, preserveScroll: true });
    };

    return (
        <AdminLayout>
            <Head title="Colorstone colors" />

            <div className="space-y-8">
                <div className="flex items-center justify-between rounded-3xl bg-white p-6 shadow-xl shadow-slate-900/10 ring-1 ring-slate-200/80">
                    <div>
                        <h1 className="text-2xl font-semibold text-slate-900">Colorstone colors</h1>
                        <p className="mt-2 text-sm text-slate-500">Manage colorstone color grades for catalogue specifications.</p>
                    </div>
                    <button
                        type="button"
                            ...
                            <tr>
                                <th className="px-5 py-3">
                                    <input
                                        ...
                                        aria-label="Select all colorstone colors"
                                    />
                                </th>
                                ...
                                <tr key={color.id} className="hover:bg-slate-50">
                                    <td className="px-5 py-3">
                                        <input
                                            ...
                                            className="h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500"
                                            aria-label={`Select colorstone color ${color.name}`}
                                        />
                                    </td>
                            ...
                            {colors.data.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="px-5 py-6 text-center text-sm text-slate-500">
                                        No colorstone colors defined yet.
                                    </td>
                                </tr>
                            )}
                        </tbody>
                    ...
                    <div className="flex-shrink-0 border-b border-slate-200 px-6 py-4">
                        <div className="flex items-center justify-between">
                            <h2 className="text-xl font-semibold text-slate-900">
                                {editingColor ? `Edit colorstone color: ${editingColor.name}` : 'Create new colorstone color'}
                            </h2>
                            <div className="flex items-center gap-3">
                                ...
                                >
                                    {editingColor ? 'Update colorstone color' : 'Create colorstone color'}
                                </button>
                                <button
                                    type="button"
                                    onClick={resetForm}
            ...
            <ConfirmationModal
                ...
                title="Remove Colorstone Color"
                message={deleteConfirm ? `Are you sure you want to remove colorstone color ${deleteConfirm.name}?` : ''}
                confirmText="Remove"
                variant="danger"
            />
            ...
            <ConfirmationModal
                ...
                title="Delete Colorstone Colors"
                message={`Are you sure you want to delete ${selectedColors.length} selected colorstone color(s)?`}
                confirmText="Delete"
                variant="danger"
            />
        </AdminLayout>
