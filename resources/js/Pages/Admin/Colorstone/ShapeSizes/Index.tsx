...
type ColorstoneShapeSizesPageProps = PageProps<{
    sizes: Pagination<ColorstoneShapeSizeRow>;
    shapes: ColorstoneShape[];
    selectedShapeId?: number;
}>;

export default function AdminColorstoneShapeSizesIndex() {
    const { sizes, shapes, selectedShapeId } = usePage<ColorstoneShapeSizesPageProps>().props;
    ...
    const form = useForm({
        colorstone_shape_id: selectedShapeId || (shapes.length > 0 ? shapes[0].id : 0),
        size: '',
        secondary_size: '',
        description: '',
        display_order: 0,
    ...
    const resetForm = () => {
        ...
        setModalOpen(false);
        form.reset();
        form.setData('colorstone_shape_id', selectedShapeId || (shapes.length > 0 ? shapes[0].id : 0));
    ...
    const openEditModal = (size: ColorstoneShapeSizeRow) => {
        setEditingSize(size);
        form.setData({
    ...
    const submit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (editingSize) {
            form.put(route('admin.colorstone.shape-sizes.update', editingSize.id), {
                preserveScroll: true,
                onSuccess: () => resetForm(),
            });
        } else {
            form.post(route('admin.colorstone.shape-sizes.store'), {
                preserveScroll: true,
                onSuccess: () => resetForm(),
            });
        }
    ...
    const deleteSize = (size: ColorstoneShapeSizeRow) => {
        setDeleteConfirm(size);
    };

    const handleDelete = () => {
        if (deleteConfirm) {
            router.delete(route('admin.colorstone.shape-sizes.destroy', deleteConfirm.id), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const handleBulkDelete = () => {
        router.delete(route('admin.colorstone.shape-sizes.bulk-destroy'), {
    ...
    const handlePerPageChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        router.get(route('admin.colorstone.shape-sizes.index'), { per_page: newPerPage, shape_id: selectedShapeId }, { preserveState: true, preserveScroll: true });
    };
    ...
    const handleShapeFilter = (event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        router.get(route('admin.colorstone.shape-sizes.index'), { shape_id: shapeId }, { preserveState: true, preserveScroll: true });
    };

    return (
        <AdminLayout>
            <Head title="Colorstone shape sizes" />

            <div className="space-y-8">
                <div className="flex items-center justify-between rounded-3xl bg-white p-6 shadow-xl shadow-slate-900/10 ring-1 ring-slate-200/80">
                    <div>
                        <h1 className="text-2xl font-semibold text-slate-900">Colorstone shape sizes</h1>
                        <p className="mt-2 text-sm text-slate-500">Manage colorstone shape sizes and carat weights for catalogue specifications.</p>
                    </div>
                    <button
                        type="button"
                            ...
                            <tr>
                                <th className="px-5 py-3">
                                    <input
                                        ...
                                        aria-label="Select all colorstone shape sizes"
                                    />
                                </th>
                                ...
                                <tr key={size.id} className="hover:bg-slate-50">
                                    <td className="px-5 py-3">
                                        <input
                                            ...
                                            className="h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500"
                                            aria-label={`Select colorstone shape size ${size.size}`}
                                        />
                                    </td>
                            ...
                            {sizes.data.length === 0 && (
                                <tr>
                                    <td colSpan={7} className="px-5 py-6 text-center text-sm text-slate-500">
                                        No colorstone shape sizes defined yet.
                                    </td>
                                </tr>
                            )}
                        </tbody>
                    ...
                    <div className="flex-shrink-0 border-b border-slate-200 px-6 py-4">
                        <div className="flex items-center justify-between">
                            <h2 className="text-xl font-semibold text-slate-900">
                                {editingSize ? `Edit colorstone shape size` : 'Create new colorstone shape size'}
                            </h2>
                            <div className="flex items-center gap-3">
                                ...
                                >
                                    {editingSize ? 'Update colorstone shape size' : 'Create colorstone shape size'}
                                </button>
                                <button
                                    type="button"
                                    onClick={resetForm}
                                ...
                                <div className="space-y-6">
                                    <div className="grid gap-4">
                                        <label className="flex flex-col gap-2 text-sm text-slate-600">
                                            <span>Colorstone Shape *</span>
                                            <select
                                                value={form.data.colorstone_shape_id}
                                                onChange={(event) => form.setData('colorstone_shape_id', Number(event.target.value))}
            ...
            <ConfirmationModal
                ...
                title="Remove Colorstone Shape Size"
                message={deleteConfirm ? `Are you sure you want to remove this colorstone shape size?` : ''}
                confirmText="Remove"
                variant="danger"
            />
            ...
            <ConfirmationModal
                ...
                title="Delete Colorstone Shape Sizes"
                message={`Are you sure you want to delete ${selectedSizes.length} selected colorstone shape size(s)?`}
                confirmText="Delete"
                variant="danger"
            />
        </AdminLayout>
