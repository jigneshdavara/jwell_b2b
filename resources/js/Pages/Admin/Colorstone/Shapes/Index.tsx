...
type ColorstoneShapesPageProps = PageProps<{
    shapes: Pagination<ColorstoneShapeRow>;
}>;

export default function AdminColorstoneShapesIndex() {
    const { shapes } = usePage<ColorstoneShapesPageProps>().props;
    ...
    const openEditModal = (shape: ColorstoneShapeRow) => {
    ...
    const submit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (editingShape) {
            form.put(route('admin.colorstone.shapes.update', editingShape.id), {
                preserveScroll: true,
                onSuccess: () => resetForm(),
            });
        } else {
            form.post(route('admin.colorstone.shapes.store'), {
                preserveScroll: true,
                onSuccess: () => resetForm(),
            });
        }
    ...
    const toggleShape = (shape: ColorstoneShapeRow) => {
        router.put(route('admin.colorstone.shapes.update', shape.id), {
            code: shape.code,
            name: shape.name,
                        description: shape.description,
            is_active: !shape.is_active,
    ...
    const deleteShape = (shape: ColorstoneShapeRow) => {
        setDeleteConfirm(shape);
    };

    const handleDelete = () => {
        if (deleteConfirm) {
            router.delete(route('admin.colorstone.shapes.destroy', deleteConfirm.id), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const handleBulkDelete = () => {
        router.delete(route('admin.colorstone.shapes.bulk-destroy'), {
    ...
    const handlePerPageChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        router.get(route('admin.colorstone.shapes.index'), { per_page: newPerPage }, { preserveState: true, preserveScroll: true });
    };

    return (
        <AdminLayout>
            <Head title="Colorstone shapes" />

            <div className="space-y-8">
                <div className="flex items-center justify-between rounded-3xl bg-white p-6 shadow-xl shadow-slate-900/10 ring-1 ring-slate-200/80">
                    <div>
                        <h1 className="text-2xl font-semibold text-slate-900">Colorstone shapes</h1>
                        <p className="mt-2 text-sm text-slate-500">Manage colorstone shapes for catalogue specifications.</p>
                    </div>
                    <button
                        type="button"
                            ...
                            <tr>
                                <th className="px-5 py-3">
                                    <input
                                        ...
                                        aria-label="Select all colorstone shapes"
                                    />
                                </th>
                                ...
                                <tr key={shape.id} className="hover:bg-slate-50">
                                    <td className="px-5 py-3">
                                        <input
                                            ...
                                            className="h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500"
                                            aria-label={`Select colorstone shape ${shape.name}`}
                                        />
                                    </td>
                            ...
                            {shapes.data.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="px-5 py-6 text-center text-sm text-slate-500">
                                        No colorstone shapes defined yet.
                                    </td>
                                </tr>
                            )}
                        </tbody>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-slate-600">
                    <div>
                        Showing {shapes.from ?? 0} to {shapes.to ?? 0} of {shapes.total} entries
                    </div>
                    ...
                    <div className="flex-shrink-0 border-b border-slate-200 px-6 py-4">
                        <div className="flex items-center justify-between">
                            <h2 className="text-xl font-semibold text-slate-900">
                                {editingShape ? `Edit colorstone shape: ${editingShape.name}` : 'Create new colorstone shape'}
                            </h2>
                            <div className="flex items-center gap-3">
                                ...
                                >
                                    {editingShape ? 'Update colorstone shape' : 'Create colorstone shape'}
                                </button>
                                <button
                                    type="button"
                                    onClick={resetForm}
            ...
            <ConfirmationModal
                ...
                title="Remove Colorstone Shape"
                message={deleteConfirm ? `Are you sure you want to remove colorstone shape ${deleteConfirm.name}?` : ''}
                confirmText="Remove"
                variant="danger"
            />
            ...
            <ConfirmationModal
                ...
                title="Delete Colorstone Shapes"
                message={`Are you sure you want to delete ${selectedShapes.length} selected colorstone shape(s)?`}
                confirmText="Delete"
                variant="danger"
            />
        </AdminLayout>
