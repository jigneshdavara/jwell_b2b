...
type MetalTonesPageProps = PageProps<{
    tones: Pagination<MetalToneRow>;
    metals: MetalOption[];
}>;

export default function AdminMetalTonesIndex() {
    const { tones, metals } = usePage<MetalTonesPageProps>().props;
    ...
    useEffect(() => {
        const existingIds = new Set(tones.data.map((tone) => tone.id));
        setSelectedTones((prev) => prev.filter((id) => existingIds.has(id)));
    }, [tones.data]);

    const allSelected = useMemo(() => {
        if (tones.data.length === 0) {
            return false;
        }
        return selectedTones.length === tones.data.length;
    }, [tones.data, selectedTones]);
    ...
    const submit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (editingTone) {
            form.put(route('admin.metal-tones.update', editingTone.id), {
                preserveScroll: true,
                onSuccess: () => resetForm(),
            });
        } else {
            form.post(route('admin.metal-tones.store'), {
                preserveScroll: true,
                onSuccess: () => resetForm(),
            });
        }
    ...
    const deleteTone = (tone: MetalToneRow) => {
        setDeleteConfirm(tone);
    };

    const handleDelete = () => {
        if (deleteConfirm) {
            router.delete(route('admin.metal-tones.destroy', deleteConfirm.id), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const handleBulkDelete = () => {
        router.delete(route('admin.metal-tones.bulk-destroy'), {
            data: { ids: selectedTones },
            preserveScroll: true,
            onSuccess: () => {
                setSelectedTones([]);
    ...
    const handlePerPageChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        router.get(route('admin.metal-tones.index'), { per_page: newPerPage }, { preserveState: true, preserveScroll: true });
    };
        ...
        <AdminLayout>
            ...
            <div className="space-y-8">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-slate-600">
                    <div>
                        Showing {tones.from ?? 0} to {tones.to ?? 0} of {tones.total} entries
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {tones.links.map((link, index) => {
