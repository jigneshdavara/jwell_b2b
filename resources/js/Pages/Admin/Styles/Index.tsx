...
type StylesPageProps = PageProps<{
    styles: Pagination<StyleRow>;
}>;

export default function AdminStylesIndex() {
    const { styles } = usePage<StylesPageProps>().props;
    const [modalOpen, setModalOpen] = useState(false);
    const [editingStyle, setEditingStyle] = useState<StyleRow | null>(null);
    const [selectedStyles, setSelectedStyles] = useState<number[]>([]);
    const [perPage, setPerPage] = useState(styles.per_page ?? 10);
    ...
    useEffect(() => {
        const existingIds = new Set(styles.data.map((style) => style.id));
        setSelectedStyles((prev) => prev.filter((id) => existingIds.has(id)));
    }, [styles.data]);
    ...
    const allSelected = useMemo(() => {
        ...
        return selectedStyles.length === styles.data.length;
    ...
    const submit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (editingStyle) {
            form.put(route('admin.styles.update', editingStyle.id), {
                preserveScroll: true,
                onSuccess: () => resetForm(),
            });
        } else {
            form.post(route('admin.styles.store'), {
                preserveScroll: true,
                onSuccess: () => resetForm(),
            });
        }
    ...
    const toggleStyle = (style: StyleRow) => {
        router.put(route('admin.styles.update', style.id), {
            code: style.code,
            name: style.name,
            description: style.description,
            is_active: !style.is_active,
    ...
    const deleteStyle = (style: StyleRow) => {
        setDeleteConfirm(style);
    };

    const handleDelete = () => {
        if (deleteConfirm) {
            router.delete(route('admin.styles.destroy', deleteConfirm.id), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const handleBulkDelete = () => {
        router.delete(route('admin.styles.bulk-destroy'), {
            data: { ids: selectedStyles },
            preserveScroll: true,
            onSuccess: () => {
                setSelectedStyles([]);
    ...
    const handlePerPageChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        router.get(route('admin.styles.index'), { per_page: newPerPage }, { preserveState: true, preserveScroll: true });
    };
        ...
        <AdminLayout>
            ...
            <div className="space-y-8">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-slate-600">
                    <div>
                        Showing {styles.from ?? 0} to {styles.to ?? 0} of {styles.total} entries
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {styles.links.map((link, index) => {
