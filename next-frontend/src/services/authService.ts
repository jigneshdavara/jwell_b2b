...
export const authService = {
    ...
    async logout() {
        if (typeof window !== "undefined") {
            // CRITICAL: Set logout flag FIRST, before removing token
            // This prevents new API requests from being initiated
            (window as any).__isLoggingOut = true;

            // Cancel all pending API requests
            // Import synchronously to avoid async delay
            const { cancelAllRequests } = require("./api");
            cancelAllRequests();

            // Remove token after flag is set and requests are cancelled
            ...
            // Clear the flag after a short delay (in case redirect is delayed)
            setTimeout(() => {
