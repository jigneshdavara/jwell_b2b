...
import { metalSchema, MetalFormData } from "@/lib/validation/admin.schema";
...
export default function AdminMetalsPage() {
    ...
    const onSubmit = async (data: MetalFormData) => {
        setLoading(true);
        try {
            const payload = {
                code: data.code,
            ...
            if (error.response?.data?.errors) {
                ...
                Object.keys(fieldErrors).forEach((key) => {
                    const errorMessage = Array.isArray(fieldErrors[key]) 
                        ? fieldErrors[key][0] 
                        : fieldErrors[key];
                    setError(key as keyof MetalFormData, {
                        type: 'server',
            ...
            const errorMessage = error.response?.data?.message 
                ...
                : 'Failed to save metal. Please try again.';
            ...
            <Modal show={modalOpen} onClose={resetForm} maxWidth="5xl">
                <div className="flex min-h-0 flex-col">
                    ...
                    <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3 sm:px-6 sm:py-4">
                        <form onSubmit={handleFormSubmit(onSubmit)} className="space-y-4 sm:space-y-6" id="metal-form">
