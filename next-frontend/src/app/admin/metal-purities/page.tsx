...
export default function AdminMetalPuritiesIndex() {
    ...
    const openEditModal = (purity: MetalPurityRow) => {
        ...
        setFormData({ name: purity.name, is_active: purity.is_active });
        setModalOpen(true);
    };
    ...
    const submit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            if (editingPurity) {
                await adminService.updateMetalPurity(editingPurity.id, formData);
            } else {
                await adminService.createMetalPurity(formData);
            }
            resetFormAndModal();
            await loadPurities();
        } catch (error: any) {
    ...
    const toggleStatus = async (purity: MetalPurityRow) => {
        try {
            await adminService.updateMetalPurity(purity.id, { ...purity, is_active: !purity.is_active });
            await loadPurities();
        } catch (error: any) {
            ...
            <div className="space-y-8">
                ...
                <div className="overflow-hidden rounded-3xl bg-white shadow-xl shadow-slate-900/10 ring-1 ring-slate-200/80">
                    <div className="flex items-center justify-between gap-4 border-b border-slate-200 px-5 py-4 text-sm">
                        <div className="font-semibold text-slate-700">Purities ({purities.meta.total})</div>
                        <select
                            value={perPage}
                            onChange={(e) => {
                    ...
                    {loading && purities.data.length === 0 ? (
                        <div className="flex items-center justify-center py-12">
                            <p className="text-slate-500">Loading metal purities...</p>
                        </div>
                    ) : (
                                    ...
                                    <tr key={purity.id} className="hover:bg-slate-50">
                                        ...
                                        <td className="px-5 py-3 text-right">
                                            <div className="flex justify-end gap-2">
                                                <button type="button" onClick={() => openEditModal(purity)} className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:border-slate-300 hover:text-slate-900">
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
                                                        <path strokeLinecap="round" strokeLinejoin="round" d="M4 16.5V19a1 1 0 001 1h2.5a1 1 0 00.7-.3l9.8-9.8a1 1 0 000-1.4l-2.5-2.5a1 1 0 00-1.4 0l-9.8 9.8a1 1 0 00-.3.7z" />
                                                        <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 6.5l4 4" />
                                                    </svg>
                                                </button>
                                                <button type="button" onClick={() => toggleStatus(purity)} className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:border-amber-200 hover:text-amber-600">
                                                    {purity.is_active ? (
                                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
                                                ...
                                                <button type="button" onClick={() => setDeleteConfirm(purity)} className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-rose-200 text-rose-500 transition hover:border-rose-300 hover:bg-rose-50 hover:text-rose-600">
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
                            ...
                            {Array.from({ length: purities.meta.last_page }, (_, i) => i + 1).map((page) => (
                                <button
                                    ...
                                    className={`rounded-full px-3 py-1 text-sm font-semibold transition ${
                                        page === purities.meta.current_page ? 'bg-sky-600 text-white shadow shadow-sky-600/20' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                                    }`}
                                >
                                    {page}
                                </button>
                ...
                <Modal show={modalOpen} onClose={resetFormAndModal} maxWidth="xl">
                    <form onSubmit={submit} className="p-6">
                        <h3 className="text-lg font-semibold text-slate-900">{editingPurity ? 'Edit Metal Purity' : 'New Metal Purity'}</h3>
