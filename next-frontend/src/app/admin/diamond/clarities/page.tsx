...
export default function AdminDiamondClaritiesIndex() {
        ...
        try {
            ...
            setClarities({
                ...
                meta: {
                    ...
                    from: responseMeta.from ?? ((responseMeta.current_page || responseMeta.page || currentPage) - 1) * (responseMeta.per_page || responseMeta.perPage || perPage) + 1,
                    to: responseMeta.to ?? Math.min((responseMeta.current_page || responseMeta.page || currentPage) * (responseMeta.per_page || responseMeta.perPage || perPage), responseMeta.total || 0),
    ...
    const handleBulkDelete = async () => {
        try {
            await adminService.bulkDeleteDiamondClarities(selectedClarities);
            setSelectedClarities([]);
            setBulkDeleteConfirm(false);
            await loadClarities();
        } catch (error: any) {
            console.error('Failed to bulk delete diamond clarities:', error);
            alert(error.response?.data?.message || 'Failed to delete diamond clarities. Please try again.');
        }
    };
    ...
    const changePage = (url: string | null) => {
        ...
        try {
            ...
            if (url.startsWith('http') || url.startsWith('/')) {
                ...
                // Try to extract page number directly
            ...
            console.error('Failed to parse pagination URL:', error);
        }
    };
        ...
        <>
            ...
            <div className="space-y-8">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-slate-600">
                    <div>
                        Showing {clarities.meta.from ?? 0} to {clarities.meta.to ?? 0} of {clarities.meta.total} entries
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {clarities.meta.links?.map((link, index) => {
