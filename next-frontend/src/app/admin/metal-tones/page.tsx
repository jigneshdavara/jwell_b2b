...
import { metalToneSchema, MetalToneFormData } from '@/lib/validation/admin.schema';
...
export default function AdminMetalTonesIndex() {
    ...
    const {
        ...
        setError,
        trigger,
    } = useForm<MetalToneFormData>({
        resolver: zodResolver(metalToneSchema),
        mode: "onSubmit",
    ...
    const onSubmit = async (data: MetalToneFormData) => {
        setProcessing(true);
        try {
            const payload = {
            ...
            if (error.response?.data?.errors) {
                ...
                Object.keys(fieldErrors).forEach((key) => {
                    const errorMessage = Array.isArray(fieldErrors[key]) 
                        ? fieldErrors[key][0] 
                        : fieldErrors[key];
                    setError(key as keyof MetalToneFormData, {
                        type: 'server',
            ...
            const errorMessage = error.response?.data?.message 
                ...
                : 'Failed to save metal tone. Please try again.';
        ...
        <>
            ...
            <div className="space-y-4 px-2 py-4 sm:space-y-6 sm:px-6 sm:py-6 lg:space-y-8 lg:px-8">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-3xl bg-white p-4 sm:p-6 shadow-xl shadow-slate-900/10 ring-1 ring-slate-200/80">
                    <div>
                        <h1 className="text-xl sm:text-2xl font-semibold text-slate-900">Metal tones</h1>
                        <p className="mt-2 text-xs sm:text-sm text-slate-500">Manage metal tone variations for catalogue specifications.</p>
                    </div>
                    <button
                        type="button"
                ...
                <div className="overflow-hidden rounded-3xl bg-white shadow-xl shadow-slate-900/10 ring-1 ring-slate-200/80">
                    ...
                    {loading && tones.data.length === 0 ? (
                        <div className="flex items-center justify-center py-8 sm:py-12">
                            <p className="text-xs sm:text-sm text-slate-500">Loading metal tones...</p>
                        </div>
                    ) : (
                        <div className="overflow-x-auto">
                            ...
                            <thead className="bg-slate-50 text-xs text-slate-500">
                                <tr>
                                        ...
                                        <th className="px-3 py-2 text-left sm:px-5 sm:py-3">Tone name</th>
                                ...
                                {tones.data.length === 0 ? (
                                    ...
                                    tones.data.map((tone) => (
                                        <tr key={tone.id} className="hover:bg-slate-50">
                                                ...
                                                <td className="px-3 py-2 text-slate-700 sm:px-5 sm:py-3">{tone.code}</td>
                                                <td className="px-3 py-2 font-semibold text-slate-900 sm:px-5 sm:py-3">
                                                <div className="flex flex-col gap-1">
                                                        <span className="text-xs sm:text-sm">{tone.name}</span>
                                                        {tone.description && <span className="text-[10px] sm:text-xs text-slate-500">{tone.description}</span>}
                                                </div>
                                            </td>
                                                <td className="px-3 py-2 text-slate-500 hidden lg:table-cell sm:px-5 sm:py-3">{tone.display_order}</td>
                                                <td className="px-3 py-2 sm:px-5 sm:py-3">
                ...
                <Modal show={modalOpen} onClose={resetFormAndModal} maxWidth="5xl">
                    <div className="flex min-h-0 flex-col">
                        ...
                        <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3 sm:px-6 sm:py-4">
                            <form onSubmit={handleFormSubmit(onSubmit)} className="space-y-4 sm:space-y-6" id="tone-form">
                                <div className="grid gap-4 sm:gap-6 lg:grid-cols-2">
                                    <div className="space-y-4 sm:space-y-6">
                                        <div className="grid gap-3 sm:gap-4">
                                            {/* Metal */}
                                            <div className="flex flex-col gap-1.5 sm:gap-2">
                                                <InputLabel htmlFor="metal_id">
                                                    Metal <span className="text-rose-500">*</span>
                                                </InputLabel>
                                                <Controller
                                                    name="metal_id"
                                                    control={control}
                                            ...
                                            <div className="flex flex-col gap-1.5 sm:gap-2">
                                                <InputLabel htmlFor="name">
                                                    Tone name <span className="text-rose-500">*</span>
                                                </InputLabel>
                                                <Controller
                                                    name="name"
                                                    control={control}
