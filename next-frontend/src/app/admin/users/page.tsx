...
export default function AdminTeamUsersIndex() {
    ...
    const [users, setUsers] = useState<{ data: TeamUser[]; meta: PaginationMeta }>({
        data: [],
    ...
    const updateUserGroup = async (user: TeamUser, groupId: string) => {
        if (isProtected(user)) {
            return;
        }
        try {
            const groupIdNum = groupId ? Number(groupId) : null;
            await adminService.updateTeamUserGroup(user.id, groupIdNum);
            await loadUsers();
        } catch (error: any) {
    ...
    const submitNewUser = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (editingUser) {
                ...
                await adminService.updateTeamUser(editingUser.id, payload);
            } else {
                payload.password = newUser.password;
                payload.password_confirmation = newUser.password_confirmation;
                await adminService.createTeamUser(payload);
            }
    ...
    const handleDelete = async () => {
        if (deleteConfirm) {
            try {
                await adminService.deleteTeamUser(deleteConfirm.id);
    ...
    const handleBulkDelete = async () => {
        try {
            await adminService.bulkDeleteTeamUsers(selectedIds);
            setSelectedIds([]);
