...
const mockDiscounts: DiscountRow[] = [
    { id: 1, name: 'Retailer Standard', type: 'percentage', value: 5, is_active: true },
    { id: 2, name: 'Wholesaler Bulk', type: 'percentage', value: 15, is_active: true },
];

export default function AdminMakingChargeDiscountsIndex() {
    const [allDiscounts, setAllDiscounts] = useState<DiscountRow[]>(mockDiscounts);
    ...
    const paginatedDiscounts = useMemo(() => {
        ...
        return allDiscounts.slice(start, start + perPage);
    }, [allDiscounts, currentPage, perPage]);
    ...
    const submit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        setTimeout(() => {
            if (editingDiscount) {
                setAllDiscounts(allDiscounts.map(d => d.id === editingDiscount.id ? { ...d, ...formData } : d));
            } else {
                setAllDiscounts([...allDiscounts, { id: Date.now(), ...formData }]);
            }
            setProcessing(false);
            resetFormAndModal();
        }, 500);
    ...
    const toggleStatus = (discount: DiscountRow) => {
        setAllDiscounts(allDiscounts.map(d => d.id === discount.id ? { ...d, is_active: !d.is_active } : d));
    };

    const handleDelete = () => {
        if (deleteConfirm) {
            setAllDiscounts(allDiscounts.filter(d => d.id !== deleteConfirm.id));
            ...
            <div className="space-y-8">
                ...
                <div className="overflow-hidden rounded-3xl bg-white shadow-xl shadow-slate-900/10 ring-1 ring-slate-200/80">
                    <div className="flex items-center justify-between gap-4 border-b border-slate-200 px-5 py-4 text-sm">
                        <div className="font-semibold text-slate-700">Discounts ({allDiscounts.length})</div>
                    </div>
