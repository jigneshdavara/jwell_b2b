...
export default function AdminDashboardOverview() {
  const [metrics, setMetrics] = useState<Record<string, number>>({
    pending_kyc: 12,
    orders_in_production: 45,
    active_offers: 8,
  });
  const [recentPartners, setRecentPartners] = useState<Partner[]>([
    { id: 1, name: 'Jewel House', email: 'owner@jewelhouse.com', type: 'Retailer', kyc_status: 'pending', joined_at: new Date().toISOString() },
    { id: 2, name: 'Royal Gems', email: 'contact@royalgems.in', type: 'Wholesaler', kyc_status: 'approved', joined_at: new Date().toISOString() },
  ]);
  ...
  return (
    <div className="space-y-8">
        <div className="flex flex-col gap-4 rounded-3xl bg-white p-6 shadow-xl shadow-slate-900/5 ring-1 ring-slate-200/70 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.1em] text-slate-500/80">
              Elvee HQ
            </p>
            <h1 className="mt-2 text-3xl font-semibold text-slate-900">
              Administration Control Tower
            </h1>
            <p className="mt-2 text-sm text-slate-500">
              Monitor partner onboarding, production flows, and commercial
              levers at a glance.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          {Object.entries(metrics).map(([key, value]) => (
            <div
              key={key}
              className="rounded-2xl bg-slate-900 p-5 text-slate-100 shadow-lg shadow-slate-900/30"
            >
            ...
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold text-slate-900">
                  Latest Partner Registrations
                </h2>
                <p className="text-[11px] uppercase tracking-[0.2em] text-slate-500/70">
                  ...
                  <div className="text-right">
                    <span
                      className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${
                        statusColors[partner.kyc_status] ??
