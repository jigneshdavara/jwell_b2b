...
import { metalPuritySchema, MetalPurityFormData } from '@/lib/validation/admin.schema';
...
export default function AdminMetalPuritiesIndex() {
    ...
    const {
        ...
        setError,
        trigger,
    } = useForm<MetalPurityFormData>({
        resolver: zodResolver(metalPuritySchema),
        mode: "onSubmit",
    ...
    const onSubmit = async (data: MetalPurityFormData) => {
        setProcessing(true);
        try {
            const payload = {
            ...
            if (error.response?.data?.errors) {
                ...
                Object.keys(fieldErrors).forEach((key) => {
                    const errorMessage = Array.isArray(fieldErrors[key]) 
                        ? fieldErrors[key][0] 
                        : fieldErrors[key];
                    setError(key as keyof MetalPurityFormData, {
                        type: 'server',
            ...
            const errorMessage = error.response?.data?.message 
                ...
                : 'Failed to save metal purity. Please try again.';
                ...
                <Modal show={modalOpen} onClose={resetFormAndModal} maxWidth="5xl">
                    <div className="flex min-h-0 flex-col">
                        ...
                        <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3 sm:px-6 sm:py-4">
                            <form onSubmit={handleFormSubmit(onSubmit)} className="space-y-4 sm:space-y-6" id="purity-form">
                                        ...
                                        <div className="grid gap-3 sm:gap-4">
                                            ...
                                            <div className="flex flex-col gap-1.5 sm:gap-2">
                                                <InputLabel htmlFor="name">
                                                    Purity name <span className="text-rose-500">*</span>
                                                </InputLabel>
                                                <Controller
                                                    name="name"
                                                    control={control}
