...
import { diamondClaritySchema, DiamondClarityFormData } from '@/lib/validation/admin.schema';
...
export default function AdminDiamondClaritiesIndex() {
    ...
    const {
        ...
        setError,
        trigger,
    } = useForm<DiamondClarityFormData>({
        resolver: zodResolver(diamondClaritySchema),
        mode: "onSubmit",
            ...
            setClarities({
                data: items
                    .filter((item: any) => {
                        ...
                        // Only show clarities from active types
    ...
    const onSubmit = async (data: DiamondClarityFormData) => {
        setProcessing(true);
        ...
        try {
            ...
            if (error.response?.data?.errors) {
                ...
                Object.keys(fieldErrors).forEach((key) => {
                    const errorMessage = Array.isArray(fieldErrors[key]) 
                        ? fieldErrors[key][0] 
                        : fieldErrors[key];
                    setError(key as keyof DiamondClarityFormData, {
                        type: 'server',
            ...
            const errorMessage = error.response?.data?.message 
                ...
                : 'Failed to save diamond clarity. Please try again.';
        ...
        <>
            ...
            <div className="space-y-4 px-2 py-4 sm:space-y-6 sm:px-6 sm:py-6 lg:space-y-8 lg:px-8">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-3xl bg-white p-4 sm:p-6 shadow-xl shadow-slate-900/10 ring-1 ring-slate-200/80">
                    <div>
                        <h1 className="text-xl sm:text-2xl font-semibold text-slate-900">Diamond clarities</h1>
                        <p className="mt-2 text-xs sm:text-sm text-slate-500">Manage diamond clarity grades for catalogue specifications.</p>
                    </div>
                    <button
                        type="button"
                        ...
                        <tbody className="divide-y divide-slate-100 bg-white">
                                ...
                                <tr key={clarity.id} className="hover:bg-slate-50">
                                        ...
                                        />
                                            ...
                                            <td className="px-3 py-2 text-slate-700 hidden md:table-cell sm:px-5 sm:py-3">{clarity.type ? clarity.type.name : '-'}</td>
                                            <td className="px-3 py-2 text-slate-700 sm:px-5 sm:py-3">{clarity.code}</td>
                                            <td className="px-3 py-2 font-semibold text-slate-900 sm:px-5 sm:py-3">
                                        <div className="flex flex-col gap-1">
                                                    <span className="text-xs sm:text-sm">{clarity.name}</span>
                                                    {clarity.description && <span className="text-[10px] sm:text-xs text-slate-500">{clarity.description}</span>}
                                        </div>
                                    </td>
                                            <td className="px-3 py-2 text-slate-500 hidden lg:table-cell sm:px-5 sm:py-3">{clarity.display_order}</td>
                                            <td className="px-3 py-2 sm:px-5 sm:py-3">
                                        <span
            ...
            <Modal show={modalOpen} onClose={resetForm} maxWidth="5xl">
                <div className="flex min-h-0 flex-col">
                    ...
                    <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3 sm:px-6 sm:py-4">
                        <form onSubmit={handleFormSubmit(onSubmit)} className="space-y-4 sm:space-y-6" id="clarity-form">
