...
import { sizeSchema, SizeFormData } from '@/lib/validation/admin.schema';
            ...
            if (error.response?.data?.errors) {
                ...
                Object.keys(fieldErrors).forEach((key) => {
                    const errorMessage = Array.isArray(fieldErrors[key]) 
                        ? fieldErrors[key][0] 
                        : fieldErrors[key];
                    setError(key as keyof SizeFormData, {
                        type: 'server',
        ...
        <>
            ...
            <div className="space-y-4 px-2 py-4 sm:space-y-6 sm:px-6 sm:py-6 lg:space-y-8 lg:px-8">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-3xl bg-white p-4 sm:p-6 shadow-xl shadow-slate-900/10 ring-1 ring-slate-200/80">
                    <div>
                        <h1 className="text-xl sm:text-2xl font-semibold text-slate-900">Sizes</h1>
                        <p className="mt-2 text-xs sm:text-sm text-slate-500">Manage sizes for catalogue specifications.</p>
                    </div>
                    <button
                        type="button"
                ...
                <div className="overflow-hidden rounded-3xl bg-white shadow-xl shadow-slate-900/10 ring-1 ring-slate-200/80">
                    ...
                    {loading && sizes.data.length === 0 ? (
                        <div className="flex items-center justify-center py-8 sm:py-12">
                            <p className="text-xs sm:text-sm text-slate-500">Loading sizes...</p>
                        </div>
                    ) : (
                                ...
                                {sizes.data.length === 0 ? (
                                    ...
                                    sizes.data.map((size) => (
                                        <tr key={size.id} className="hover:bg-slate-50">
                                                ...
                                                <td className="px-3 py-2 text-slate-700 sm:px-5 sm:py-3">{size.code || '-'}</td>
                                                ...
                                                <div className="flex flex-col gap-1">
                                                        ...
                                                        {size.description && <span className="text-[10px] sm:text-xs text-slate-500">{size.description}</span>}
                                                </div>
                                            </td>
