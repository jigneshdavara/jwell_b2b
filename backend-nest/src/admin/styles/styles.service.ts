...
export class StylesService {
  ...
  async remove(id: number) {
    ...
    if (count > 0) {
      throw new BadRequestException('Cannot delete style because it is associated with one or more categories.');
    }
  ...
  async bulkRemove(ids: number[]) {
    ...
    const associatedIds = new Set(associatedStyles.map(as => as.style_id));
