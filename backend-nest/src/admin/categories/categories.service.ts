...
export class CategoriesService {
    ...
    const [items, total] = await Promise.all([
      this.prisma.categories.findMany({
        ...
        include: {
          ...
          category_styles: { include: { styles: { select: { id: true, name: true } } } },
          category_sizes: { include: { sizes: { select: { id: true, name: true } } } },
        },
  ...
  async update(id: number, dto: UpdateCategoryDto, coverImage?: string) {
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      if (dto.style_ids !== undefined) {
        await tx.category_styles.deleteMany({ where: { category_id: categoryId } });
        if (dto.style_ids.length) {
          await tx.category_styles.createMany({
            data: dto.style_ids.map(style_id => ({
      ...
      if (dto.size_ids !== undefined) {
        await tx.category_sizes.deleteMany({ where: { category_id: categoryId } });
        if (dto.size_ids.length) {
          await tx.category_sizes.createMany({
            data: dto.size_ids.map(size_id => ({
              category_id: categoryId,
  ...
  async remove(id: number) {
    ...
    return await this.prisma.categories.delete({ where: { id: categoryId } });
  }
