...
export class ProductsService {
  ...
  async create(dto: CreateProductDto, mediaFiles?: Express.Multer.File[]) {
    return await this.prisma.$transaction(async (tx) => {
      const { variants, catalog_ids, category_ids, style_ids, making_charge_types, ...productData } = dto;
  ...
  async update(id: number, dto: UpdateProductDto, mediaFiles?: Express.Multer.File[]) {
    const productRecord = await this.prisma.products.findUnique({ where: { id: BigInt(id) } });
    if (!productRecord) throw new NotFoundException('Product not found');

    return await this.prisma.$transaction(async (tx) => {
      const { variants, catalog_ids, category_ids, style_ids, making_charge_types, removed_media_ids, ...productData } = dto;
      ...
      // Sync catalogs
      await tx.catalog_products.deleteMany({ where: { product_id: product.id } });
      if (catalog_ids && catalog_ids.length > 0) {
        await tx.catalog_products.createMany({
      ...
      // Sync media
      await this.syncMedia(product.id, mediaFiles || [], removed_media_ids || [], tx);
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      for (const variant of product.product_variants) {
        const variantAttributes = {
          ...
          sku: variant.sku ? await this.generateVariantSku(variant.sku, tx) : null,
          label: variant.label,
        ...
        const newVariant = await tx.product_variants.create({ data: variantAttributes });
  ...
  async getFormOptions() {
    const [brands, categories, catalogs, diamonds, metals, metalPurities, metalTones, sizes, customerGroups] = await Promise.all([
      this.prisma.brands.findMany({ where: { is_active: true }, orderBy: { name: 'asc' } }),
      this.prisma.categories.findMany({ where: { is_active: true }, orderBy: { name: 'asc' } }),
      this.prisma.catalogs.findMany({ where: { is_active: true }, orderBy: { name: 'asc' } }),
      this.prisma.diamonds.findMany({ where: { is_active: true }, orderBy: { name: 'asc' } }),
      this.prisma.metals.findMany({ where: { is_active: true }, orderBy: { name: 'asc' } }),
      this.prisma.metal_purities.findMany({ where: { is_active: true }, include: { metals: true }, orderBy: { name: 'asc' } }),
      this.prisma.metal_tones.findMany({ where: { is_active: true }, include: { metals: true }, orderBy: { name: 'asc' } }),
      this.prisma.sizes.findMany({ where: { is_active: true }, orderBy: { name: 'asc' } }),
      this.prisma.customer_groups.findMany({ where: { is_active: true }, orderBy: { name: 'asc' } }),
    ]);
  ...
  private async syncMedia(productId: bigint, uploads: Express.Multer.File[], removedIds: number[], tx: any) {
  ...
  private deleteMediaFile(media: any) {
    ...
    if (storagePath) {
      const fullPath = path.join(process.cwd(), 'public/storage', storagePath);
      if (fs.existsSync(fullPath)) {
        try {
          fs.unlinkSync(fullPath);
        } catch (err) {
          console.error(`Failed to delete media file: ${fullPath}`, err);
        }
      }
    }
  }

  private async generateProductSku(baseSku: string, prisma: any): Promise<string> {
    let sku = baseSku;
    let exists = true;
    while (exists) {
  ...
  private async generateVariantSku(baseSku: string, prisma: any): Promise<string> {
    let sku = baseSku;
    let exists = true;
    while (exists) {
      const random = Math.random().toString(36).substring(2, 5).toUpperCase();
      sku = `${baseSku}-${random}`;
      const found = await prisma.product_variants.findUnique({ where: { sku } });
      if (!found) exists = false;
    }
    return sku;
  }
