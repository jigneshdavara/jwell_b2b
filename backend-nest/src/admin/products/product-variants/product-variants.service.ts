...
export class ProductVariantsService {
  ...
  async sync(productId: bigint, variants: ProductVariantDto[], tx?: any) {
    ...
    for (const variantDto of variants) {
      ...
      if (!metals || metals.length === 0) {
        throw new BadRequestException(`Variant "${variantDto.label}" must have at least one metal.`);
      }
      ...
      const variantAttributes = {
        ...
        size_id: variantData.size_id ? BigInt(variantData.size_id) : null,
      ...
      if (variantDto.id) {
        ...
        if (variantData.sku) {
          ...
          if (existing) {
            ...
            variantAttributes.sku = await this.generateUniqueSku(variantData.sku, prisma);
          }
        }
  ...
  private async syncMetals(variantId: bigint, metals: any[], prisma: any) {
    ...
    for (let i = 0; i < metals.length; i++) {
      ...
      const attributes = {
        ...
        metal_purity_id: metal.metal_purity_id ? BigInt(metal.metal_purity_id) : null,
        metal_tone_id: metal.metal_tone_id ? BigInt(metal.metal_tone_id) : null,
        metal_weight: metal.metal_weight ? parseFloat(metal.metal_weight.toString()) : null,
  ...
  private async syncDiamonds(variantId: bigint, diamonds: any[], prisma: any) {
  ...
  private async generateUniqueSku(baseSku: string, prisma: any): Promise<string> {
    let sku = baseSku;
    let exists = true;
    while (exists) {
