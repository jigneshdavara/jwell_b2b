...
export class QuotationsService {
  ...
  async findAllFrontend(userId: bigint) {
    const items = await this.prisma.quotations.findMany({
      ...
      include: {
        products: {
          include: { product_medias: { orderBy: { display_order: 'asc' } } },
        },
    ...
    const formattedGroups = Array.from(groups.values()).map((group) => {
      ...
      if (group.some((q) => q.status === 'pending_customer_confirmation')) {
      ...
      return {
        ...
        product: {
          ...
          thumbnail: first.products.product_medias[0]?.url || null,
        },
    ...
    return formattedGroups.sort((a, b) => b.created_at.getTime() - a.created_at.getTime());
  }

  async findOneFrontend(id: number, userId: bigint) {
      ...
      include: {
        products: {
          include: { 
            ...
            product_variants: {
              include: {
                product_variant_metals: { include: { metals: true, metal_purities: true, metal_tones: true } },
                product_variant_diamonds: { include: { diamonds: true } },
                sizes: true,
              }
            }
          },
    ...
    const groupKey = quotation.quotation_group_id;
    const relatedQuotations = await this.prisma.quotations.findMany({
      where: {
        id: { not: quotation.id },
        user_id: userId,
    ...
    const relatedFormatted = await Promise.all(relatedQuotations.map(async (q) => {
      const relatedPrice = await this.pricingService.calculateProductPrice(
        q.products,
        user,
        {
      ...
      return {
        ...
        product: {
          ...
          name: q.products.name,
          sku: q.products.sku,
          media: q.products.product_medias.map(m => ({ url: m.url })),
          variants: q.products.product_variants.map(v => ({ id: v.id.toString(), label: v.label })),
        },
        variant: q.product_variants ? { id: q.product_variants.id.toString(), label: q.product_variants.label } : null,
        price_breakdown: relatedPrice,
      };
    }));

    const taxSummary = await this.calculateTaxSummary([quotation, ...relatedQuotations], user);
    ...
    return {
      ...
      product: {
        ...
        media: quotation.products.product_medias.map(m => ({ url: m.url })),
        variants: quotation.products.product_variants.map(v => ({ id: v.id.toString(), label: v.label })),
      },
      variant: quotation.product_variants ? { id: quotation.product_variants.id.toString(), label: quotation.product_variants.label } : null,
      ...
      order: quotation.orders ? {
        ...
        history: quotation.orders.order_status_histories.map(h => ({
          id: h.id.toString(),
          status: h.status,
  ...
  async create(dto: CreateQuotationDto, userId: bigint) {
    const product = await this.prisma.products.findUnique({ where: { id: BigInt(dto.product_id) } });
    if (!product) throw new NotFoundException('Product not found');

    if (dto.product_variant_id) {
      const variant = await this.prisma.product_variants.findUnique({ where: { id: BigInt(dto.product_variant_id) } });
      if (!variant || variant.product_id !== product.id) {
        throw new BadRequestException('Invalid product variant');
      }
      if (variant.inventory_quantity !== null && variant.inventory_quantity < dto.quantity) {
  ...
  async storeFromCart(userId: bigint) {
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      for (const item of cart.cart_items) {
        if (item.product_variants && item.product_variants.inventory_quantity !== null) {
          if (item.product_variants.inventory_quantity < item.quantity) {
            throw new BadRequestException(`Insufficient inventory for ${item.products.name}`);
          }
        }
      }
  ...
  async addMessage(id: number, message: string, userId: bigint, sender: 'customer' | 'admin') {
    ...
    return await this.prisma.quotation_messages.create({
      data: {
        ...
        user_id: sender === 'customer' ? userId : null,
        sender,
        message,
        created_at: new Date(),
        updated_at: new Date(),
  ...
  async confirm(id: number, userId: bigint) {
    const quotation = await this.prisma.quotations.findUnique({ where: { id: BigInt(id) } });
    if (!quotation || quotation.user_id !== userId) throw new NotFoundException('Quotation not found');

    const related = await this.prisma.quotations.findMany({
      where: { quotation_group_id: quotation.quotation_group_id, user_id: userId }
    });

    if (!related.some(q => q.status === 'pending_customer_confirmation')) {
      throw new BadRequestException('No confirmation required');
    }

    return await this.prisma.$transaction(async (tx) => {
      await tx.quotations.updateMany({
        where: { quotation_group_id: quotation.quotation_group_id, status: 'pending_customer_confirmation' },
  ...
  // Admin Methods
  async findAllAdmin(filters: QuotationFilterDto) {
  ...
  async findOneAdmin(id: number) {
    ...
    const taxSummary = await this.calculateTaxSummary([quotation, ...related], user);
  ...
  async approve(id: number, adminNotes?: string) {
    ...
    if (quotation.status === 'approved') throw new BadRequestException('Already approved');
    if (!['pending', 'customer_confirmed'].includes(quotation.status)) {
      throw new BadRequestException('Quotation must be confirmed by customer before approval');
    }
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      for (const q of related) {
        ...
        if (q.product_variant_id) {
          const variant = await tx.product_variants.findUnique({ where: { id: q.product_variant_id } });
          if (variant && variant.inventory_quantity !== null) {
            await tx.product_variants.update({
              where: { id: q.id },
              data: { inventory_quantity: Math.max(0, (variant.inventory_quantity || 0) - q.quantity) }
            });
          }
        }
      }
  ...
  async requestConfirmation(id: number, dto: RequestConfirmationDto) {
    ...
    const related = await this.prisma.quotations.findMany({
      where: { quotation_group_id: quotation.quotation_group_id, status: { in: ['pending', 'approved', 'rejected'] } }
    });

    return await this.prisma.$transaction(async (tx) => {
      for (const q of related) {
        const updateData: any = { status: 'pending_customer_confirmation', updated_at: new Date() };
        if (dto.notes) updateData.admin_notes = dto.notes;
        
        if (q.id === quotation.id) {
          updateData.quantity = dto.quantity;
          if (dto.product_variant_id) updateData.product_variant_id = BigInt(dto.product_variant_id);
        }

        await tx.quotations.update({ where: { id: q.id }, data: updateData });

        await tx.quotation_messages.create({
          data: {
            quotation_id: q.id,
            sender: 'admin',
            message: dto.notes || 'Please review updated quotation details.',
            created_at: new Date(),
            updated_at: new Date(),
          }
        });
  ...
  async addItemAdmin(id: number, dto: AddQuotationItemDto) {
    const baseQuotation = await this.prisma.quotations.findUnique({ where: { id: BigInt(id) } });
    ...
    if (dto.product_variant_id) {
      const variant = await this.prisma.product_variants.findUnique({ where: { id: BigInt(dto.product_variant_id) } });
      if (!variant || variant.product_id !== product.id) throw new BadRequestException('Invalid variant');
      if (variant.inventory_quantity !== null && variant.inventory_quantity < dto.quantity) throw new BadRequestException('Insufficient inventory');
    }
    ...
    return await this.prisma.$transaction(async (tx) => {
      const newQ = await tx.quotations.create({
        data: {
          ...
          quotation_group_id: baseQuotation.quotation_group_id || uuidv4(),
          product_id: product.id,
          product_variant_id: dto.product_variant_id ? BigInt(dto.product_variant_id) : null,
          status: 'pending_customer_confirmation',
          quantity: dto.quantity,
      ...
      const message = dto.admin_notes || `Added new product '${product.name}' to quotation.`;
      await tx.quotation_messages.create({
        data: {
  ...
  async updateProductAdmin(id: number, dto: UpdateQuotationProductDto) {
    const quotation = await this.prisma.quotations.findUnique({ where: { id: BigInt(id) }, include: { products: true } });
    if (!quotation) throw new NotFoundException('Quotation not found');
    ...
    return await this.prisma.$transaction(async (tx) => {
      await tx.quotations.update({
        ...
        data: {
          product_id: product.id,
          product_variant_id: dto.product_variant_id ? BigInt(dto.product_variant_id) : quotation.product_variant_id,
          quantity: dto.quantity,
          status: 'pending_customer_confirmation',
      ...
      const message = dto.admin_notes || `Product changed from '${quotation.products.name}' to '${product.name}'.`;
      await tx.quotation_messages.create({
        data: {
          quotation_id: quotation.id,
          sender: 'admin',
  ...
  private async createOrderFromQuotations(quotations: any[], tx: any): Promise<any> {
      ...
      data: {
        ...
        price_breakdown: {
          items: orderItems.map(item => ({
            ...
            unit: item.pricing,
            quantity: item.quantity,
            line_subtotal: Math.round(item.line_subtotal * 100) / 100,
            line_discount: Math.round(item.line_discount * 100) / 100,
          }))
        },
        created_at: new Date(),
        updated_at: new Date(),
    ...
    for (const item of orderItems) {
      ...
      if (item.variant_id) {
        ...
        if (v) {
          variantData = { id: v.id.toString(), label: v.label, metadata: v.metadata };
        }
      }
  ...
  private formatProductForAdminQuotation(product: any) {
    return {
      ...
      variants: product.product_variants.map(v => ({
        ...
        size: v.sizes ? { id: v.sizes.id.toString(), name: v.sizes.name, value: v.sizes.value } : null,
        metals: v.product_variant_metals.map(m => ({
          id: m.id.toString(),
          metal_id: m.metal_id.toString(),
          metal: { id: m.metals.id.toString(), name: m.metals.name },
          metal_purity: { id: m.metal_purities.id.toString(), name: m.metal_purities.name },
          metal_tone: { id: m.metal_tones.id.toString(), name: m.metal_tones.name },
        })),
      })),
    };
  }
