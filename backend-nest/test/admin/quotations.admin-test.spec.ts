...
describe('Admin Quotations (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    // Setup Customer
    testCustomer = await prisma.customer.findFirst({ where: { email: 'customer.quotations@example.com' } });
    if (!testCustomer) {
    ...
    if (!brand) {
      brand = await prisma.brands.create({ data: { name: 'Test Brand', code: 'TBR' } });
    }
    let category = await prisma.categories.findFirst({ where: { name: 'Test Category' } });
    if (!category) {
      category = await prisma.categories.create({ data: { name: 'Test Category' } });
    }

    // Setup Product
    testProduct = await prisma.products.findFirst({ where: { sku: 'TEST-PROD-Q' } });
    if (!testProduct) {
  ...
  afterAll(async () => {
    await prisma.quotation_messages.deleteMany({ where: { message: { contains: 'Test' } } });
    if (testCustomer) await prisma.quotations.deleteMany({ where: { user_id: testCustomer.id } });
    if (testProduct) await prisma.products.deleteMany({ where: { id: testProduct.id } });
    await prisma.categories.deleteMany({ where: { name: 'Test Category' } });
    await prisma.brands.deleteMany({ where: { code: 'TBR' } });
    if (testCustomer) await prisma.customer.deleteMany({ where: { id: testCustomer.id } });
    await app.close();
  });
  ...
  describe('Quotation Lifecycle', () => {
    ...
    it('POST /api/quotations - Should create a quotation (Frontend)', async () => {
      // Need customer token
      const customerLogin = await authService.login({ ...testCustomer, guard: 'web' });
    ...
    it('POST /api/admin/quotations/:id/request-confirmation - Should update and request confirmation', async () => {
      const response = await request(app.getHttpServer())
        .post(`/api/admin/quotations/${quotationId}/request-confirmation`)
        .set('Authorization', `Bearer ${accessToken}`)
        .send({ quantity: 5, notes: 'Please confirm updated quantity' });
