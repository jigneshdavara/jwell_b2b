...
describe('Admin Products (e2e)', () => {
  ...
  afterAll(async () => {
    await prisma.products.deleteMany({ where: { sku: { startsWith: 'TEST_PROD' } } });
    await app.close();
  });
  ...
  describe('Product Lifecycle', () => {
    ...
    it('Setup dependencies', async () => {
      const brand = await prisma.brands.findFirst() || await prisma.brands.create({ 
        data: { name: 'Test Brand', code: 'TEST_BRAND' } 
      });
      brandId = brand.id.toString();

      const category = await prisma.categories.findFirst() || await prisma.categories.create({ 
        data: { name: 'Test Category' } 
      });
      categoryId = category.id.toString();

      const metal = await prisma.metals.findFirst() || await prisma.metals.create({ 
    ...
    it('GET /api/admin/products - Should list products', async () => {
      ...
      expect(response.body.items.some((p: any) => p.id.toString() === productId.toString())).toBe(true);
    });
